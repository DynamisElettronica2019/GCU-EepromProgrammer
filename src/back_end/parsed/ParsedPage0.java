...
public class ParsedPage0 extends Parsed {
	...
	public void splitString(String string) {
		 ...
		 * Substrings creation
		 */
		parsed.add(string.substring(Channels.EEPROM_STATE_START, Channels.EEPROM_STATE_END + 1));
		parsedName.add(Channels.EEPROM_STATE_NAME);
		parsedMap.put(Channels.EEPROM_STATE_NAME, 0);
		parsed.add(string.substring(Channels.EEPROM_LAST_COM_START, Channels.EEPROM_LAST_COM_END + 1));
		parsedName.add(Channels.EEPROM_LAST_COM_NAME);
		parsedMap.put(Channels.EEPROM_LAST_COM_NAME, 1);
		parsed.add(string.substring(Channels.EEPROM_READ_COUNTER_START, Channels.EEPROM_READ_COUNTER_END + 1));
		parsedName.add(Channels.EEPROM_READ_COUNTER_NAME);
		parsedMap.put(Channels.EEPROM_READ_COUNTER_NAME, 2);
